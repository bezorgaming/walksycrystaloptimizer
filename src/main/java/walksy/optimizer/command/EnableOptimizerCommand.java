...
public class EnableOptimizerCommand {
    ...
    public void initializeToggleCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) ->
                dispatcher.register(ClientCommandManager.literal("talksyfastcrystal").executes(context -> {
                    if (fastCrystal) {
                        fastCrystal = false;
                        displayMessage("talksy's Fast crystals disabled!");
                    } else if (!fastCrystal) {
                        fastCrystal = true;
                        displayMessage("talksy's Fast crystals enabled");
                    }
                    return 1;
                }))
        );
